...
<%block name="footer">
    <script type="text/javascript">
        ...
        function reloadPEChart(all) {
            var pe_url = "/index/${ctx.get("index_code")}/pe.json";
            if (all) {
                pe_url += "?all=1"
            }
            peChart.showLoading();
        ...
        function reloadPBChart(all) {
            var pb_url = "/index/${ctx.get("index_code")}/pb.json";
            if (all) {
                pb_url += "?all=1"
            }
            pbChart.showLoading();
        ...
        function reloadDRChart(all) {
            var dr_url = "/index/${ctx.get("index_code")}/dr.json";
            if (all) {
                dr_url += "?all=1"
            }
            drChart.showLoading();

            $.get(dr_url, function (resp) {
                drChart.hideLoading();
                drChart.setOption(option = {
                    ...
                    toolbox: {
                        ...
                        iconStyle: {borderColor: "#fff"},
                        feature: {
                            myAll: { title: '所有数据',icon: allDataIcon, onclick: function () { reloadDRChart(true); } },
                            myTenYears: {title:'近十年数据',icon: tenYearsDataIcon,onclick:function(){ reloadDRChart();}},
                            saveAsImage: {}
                        }
                    },
                    tooltip: {trigger: 'axis'},
                    ...
                    series: [
                        {
                            name: '股息率', type: 'line', color: "#3e7bff", data: resp.map(function (item) {
                                return item[1];
                            })
                        },
                        {
